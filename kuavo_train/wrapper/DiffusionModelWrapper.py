...
class CustomDiffusionModelWrapper(DiffusionModel):
    def __init__(self, config: CustomDiffusionConfigWrapper):
        ...
        if self.config.rgb_image_features:
            num_images = len(self.config.rgb_image_features)
...
class DiffusionRgbEncoder(nn.Module):
    ...
    def __init__(self, config: CustomDiffusionConfigWrapper):
        ...
        if config.depth_image_features:
            # 获取第一层卷积
        ...
        # The dummy input should take the number of image channels from `config.rgb_image_features` and it should
        # use the height and width from `config.crop_shape` if it is provided, otherwise it should use the
        # height and width from `config.rgb_image_features`.

        # Note: we have a check in the config class to make sure all images have the same shape.
        images_shape = next(iter(config.rgb_image_features.values())).shape
        if config.depth_image_features:
