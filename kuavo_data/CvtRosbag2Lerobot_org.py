            ...
            if ONLY_HALF_UP_BODY:
                if USE_LEJU_CLAW:
                    ...
                    if CONTROL_HAND_SIDE == "left" or CONTROL_HAND_SIDE == "both":
                        output_state = state[i, SLICE_ROBOT[0][0]:SLICE_ROBOT[0][-1]]
                    ...
                    if CONTROL_HAND_SIDE == "right" or CONTROL_HAND_SIDE == "both":
                        if CONTROL_HAND_SIDE == "both":
                            output_state = np.concatenate((output_state, state[i, SLICE_ROBOT[1][0]:SLICE_ROBOT[1][-1]]), axis=0)
                            ...
                            output_state = state[i, SLICE_ROBOT[1][0]:SLICE_ROBOT[1][-1]]
                            output_state = np.concatenate((output_state, claw_state[i, SLICE_CLAW[1][0]:SLICE_CLAW[1][-1]].astype(np.float32)), axis=0)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-d","--raw_dir", type=str, required=True, help="Path to raw ROS bag directory")
    parser.add_argument(
        "-n", "--num_of_bag", 
        type=int, 
        default=None,  
    ...
    port_kuavo_rosbag(raw_dir, repo_id, root=lerobot_dir,n = n, task=TASK_DESCRIPTION)